...
class WeightedProcrustesTrainer:
  def __init__(self, config, data_loader, val_data_loader=None):
    ...
    # # Freeze parameters
    # for index, para in enumerate(self.inlier_model.parameters()):
    #   if index < 2: # weight and bias in the first layer
    ...
    if config.resume is not None or config.resume_dir is not None:
      config.resume = os.path.join(self.checkpoint_dir, 'checkpoint.pth')
      config.resume_dir = self.checkpoint_dir
